...
# Documentation reference: https://developers.komodoplatform.com/basic-docs/atomicdex/atomicdex-setup/configure-mm2-json.html
...
if os.path.exists("MM2.json"):
	...
	while resp not in ["Y", "y", "N", "n"]:
		...
		resp = input("You already have an MM2.json file! Generate a new one? [Y/N]: ")
	if resp in ["Y", "y"]:
		os.rename("MM2.json", f"MM2_backup_{int(time.time())}.json")
